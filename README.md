...
- 支持重复 `order_id` 返回的友好提示。
- 客户从购物车完成页或充值页进入支付时，自动回到对应发票页加载支付区。
...
- VmShellPAY API 地址：`https://vmshell.win`
- 签名方式：`HMAC-SHA256`
- 终端类型：`auto`
- 支付场景：`auto`
- 订单号前缀：`WHMCS`
...
1. 客户打开 WHMCS 发票页，或从购物车完成页/充值页进入付款。
2. 如果当前页面不是发票页，插件会自动跳回 `viewinvoice.php?id=发票ID&payopen=vmshellpay`。
3. 客户选择 `支付宝.中国`、`支付宝.香港` 或 `微信`。
4. 插件会优先复用未过期且金额/币种/支付方式匹配的支付会话。
5. 如果没有可复用会话，插件调用 VmShellPAY 创建新的 HKD 结算订单。
6. 桌面端通常在发票页内显示二维码；移动端支付宝中国会尝试拉起收银台。
7. 客户完成支付。
8. VmShellPAY 调用支付通知地址。
9. 插件处理通知并调用 WHMCS `addInvoicePayment` 自动入账。
10. 如果客户先回到 WHMCS 页面，返回页和状态轮询会主动查询订单状态，降低回调延迟导致的未即时入账。

## 移动端说明

- 移动端默认优先选择 `alipay_cn`。
- `alipay_cn` 在移动端会使用接口返回的移动 H5/支付链接自动跳转。
- 微信内置浏览器无法稳定拉起支付宝中国，插件会提示用户用系统浏览器打开，或切换支付宝香港/微信。
...
4. 创建一张测试发票，完成桌面二维码、移动端跳转、支付通知、状态查询和退款联调。
