...
这版插件支持支付宝中国、支付宝香港、微信支付方式选择，账单页内展示二维码，支付成功后自动回写 WHMCS 发票，支持退款、退款通知、争议通知、多币种发票换算为 HKD 结算，以及购物车完成页和充值页自动回到发票页加载支付二维码。
...
4. 插件调用 VmShellPAY 创建 HKD 结算订单。
5. WHMCS 发票页内显示支付二维码，或跳转到接口返回的收银台地址。
6. 客户完成支付。
7. VmShellPAY 调用支付通知地址。
8. 插件处理通知并调用 WHMCS `addInvoicePayment` 自动入账。
9. 如果客户先回到 WHMCS 页面，返回页和状态轮询会主动查询订单状态，降低回调延迟导致的未即时入账。
...
- 购物车完成页没有二维码：确认自动跳回的发票页 URL 正确，并检查浏览器是否拦截跳转脚本。
